...
The scatterplot shows a very weak yet positive relationship between FARMS and math expectations. As the percentage of students who receive FARMS rises, the math performance increases slightly.  Its almost a flat line, so there's little correlation.
...
I would say that the model is not worth using, as the model only shows a small amount of variation.  Even though the p-value may be statistically significant, the slope is very small.
...
CROOKED BILLET EL SCH significantly outperformed the model, with a 96% score when 65% was expected.  On the other end, WHITEMARSH EL SCH has a -45.83 residual, scoring significantly lower.  These are outliers that show how the FARMS percentage by itself can't explain performance.  There are most likely other factors in play.
...
I would say that the analysis shows a weak correlation between FARMS and scores.  I would suggest a story on whether FARMS percentage data can be the sole way to asses school performance (likely not)
